...
const generateNewPropertyUniqueId = async (req, res, next) => {
  ...
  try {
    ...
    const query = `SELECT property_unique_id FROM new_property WHERE property_unique_id LIKE '${prefix}%' ORDER BY property_unique_id DESC LIMIT 1`;
    ...
    return res.status(500).json({message:'Error generating Property Unique ID', errorMsg: error});
  }
};
  module.exports = generateNewPropertyUniqueId
